...
namespace BioinformaticsSuite.ModuleTests.Models
{
    ...
    public class RestrictionDigestTests
    {
        ...
        public void TestOneEnzymeDigest()
        {
            ...
            var expectedFragments = new List<DigestFragment>
            {
                new DigestFragment("CC|GG", 0, "ACGTCC"),
                new DigestFragment("CC|GG", 6, "GGTGCACC"),
                new DigestFragment("CC|GG", 14, "GGCC"),
                new DigestFragment("CC|GG", 18, "GGACGT")
            };
            var expectedLabelledFragments = new List<LabelledDigestFragments>
            {
                new LabelledDigestFragments("test", expectedFragments)
            };
            ...
            for (int i = 0; i < expectedFragments.Count; i++)
            {
                ...
                Assert.AreEqual(expectedFragments[i].CutPosition,
                    actualLabelledFragments.First().DigestFramgments[i].CutPosition);
                Assert.AreEqual(expectedFragments[i].Fragment,
                    actualLabelledFragments.First().DigestFramgments[i].Fragment);
            }
        }
        ...
        public void TestThreeEnzymeDigest()
        {
            ...
            var expectedFragments = new List<DigestFragment>
            {
                new DigestFragment("CC|GG", 0, "AC"),
                new DigestFragment("AC|GT", 2, "GT"),
                new DigestFragment("GT|CC", 4, "CC"),
                new DigestFragment("CC|GG", 6, "GGTGCACC"),
                new DigestFragment("CC|GG", 14, "GGCC"),
                new DigestFragment("CC|GG", 18, "GGAC"),
                new DigestFragment("AC|GT", 22, "GT")
            };
            var expectedLabelledFragments = new List<LabelledDigestFragments>
            {
                new LabelledDigestFragments("test", expectedFragments)
